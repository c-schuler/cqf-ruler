...
public class BaseController {
	...
	protected IBaseResource addCommonParams(HttpServletRequest theServletRequest, final HomeRequest theRequest,
...
//			Map<String, List<String>> theAllRequestHeaders) {
//
//		ArrayList<Header> retVal = new ArrayList<Header>();
//		for (String nextKey : theAllRequestHeaders.keySet()) {
//			if (nextKey == "Authorization") {
//				for (String nextValue : theAllRequestHeaders.get(nextKey)) {
...
//	}
	...
	private IBaseResource loadAndAddConf(HttpServletRequest theServletRequest, final HomeRequest theRequest,
			final ModelMap theModel) {
	...
	private IResource loadAndAddConfDstu2(HttpServletRequest theServletRequest, final HomeRequest theRequest,
			final ModelMap theModel) {
		...
		try {
			conformance = (ca.uhn.fhir.model.dstu2.resource.Conformance) client.fetchConformance()
					.ofType(Conformance.class).execute();
		} catch (Exception e) {
			ourLog.warn("Failed to load conformance statement, error was: {}", e.toString());
			theModel.put("errorMsg",
					toDisplayError("Failed to load conformance statement, error was: " + e.toString(), e));
				...
				Collections.sort(nextRest.getResource(),
						new Comparator<ca.uhn.fhir.model.dstu2.resource.Conformance.RestResource>() {
							...
							public int compare(ca.uhn.fhir.model.dstu2.resource.Conformance.RestResource theO1,
								...
								if (retVal == 0) {
									retVal = theO1.getTypeElement().getValue()
	...
	private IBaseResource loadAndAddConfDstu3(HttpServletRequest theServletRequest, final HomeRequest theRequest,
			final ModelMap theModel) {
		CaptureInterceptor interceptor = new CaptureInterceptor();
		GenericClient client = theRequest.newClient(theServletRequest, getContext(theRequest), myConfig, interceptor);
		...
		try {
			capabilityStatement = client.fetchConformance().ofType(org.hl7.fhir.dstu3.model.CapabilityStatement.class)
					.execute();
		} catch (Exception ex) {
			ourLog.warn("Failed to load conformance statement, error was: {}", ex.toString());
			theModel.put("errorMsg",
					toDisplayError("Failed to load conformance statement, error was: " + ex.toString(), ex));
		}

		theModel.put("jsonEncodedConf",
				getContext(theRequest).newJsonParser().encodeResourceToString(capabilityStatement));
		...
		if (total > 0) {
			for (CapabilityStatementRestComponent nextRest : capabilityStatement.getRest()) {
				Collections.sort(nextRest.getResource(), new Comparator<CapabilityStatementRestResourceComponent>() {
					@Override
					public int compare(CapabilityStatementRestResourceComponent theO1,
							CapabilityStatementRestResourceComponent theO2) {
	...
	private IBaseResource loadAndAddConfR4(HttpServletRequest theServletRequest, final HomeRequest theRequest,
			final ModelMap theModel) {
		CaptureInterceptor interceptor = new CaptureInterceptor();
		GenericClient client = theRequest.newClient(theServletRequest, getContext(theRequest), myConfig, interceptor);
		...
		try {
			capabilityStatement = client.fetchConformance().ofType(org.hl7.fhir.r4.model.CapabilityStatement.class)
					.execute();
		} catch (Exception ex) {
			ourLog.warn("Failed to load conformance statement, error was: {}", ex.toString());
			theModel.put("errorMsg",
		...
		for (org.hl7.fhir.r4.model.CapabilityStatement.CapabilityStatementRestComponent nextRest : capabilityStatement
				.getRest()) {
			for (org.hl7.fhir.r4.model.CapabilityStatement.CapabilityStatementRestResourceComponent nextResource : nextRest
					.getResource()) {
				List<org.hl7.fhir.r4.model.Extension> exts = nextResource.getExtensionsByUrl(RESOURCE_COUNT_EXT_URL);
				if (exts != null && exts.size() > 0) {
					Number nextCount = ((org.hl7.fhir.r4.model.DecimalType) (exts.get(0).getValue()))
							.getValueAsNumber();
					resourceCounts.put(nextResource.getTypeElement().getValue(), nextCount);
					total += nextCount.longValue();
				}
		...
		if (total > 0) {
			for (org.hl7.fhir.r4.model.CapabilityStatement.CapabilityStatementRestComponent nextRest : capabilityStatement
					.getRest()) {
				Collections.sort(nextRest.getResource(),
						new Comparator<org.hl7.fhir.r4.model.CapabilityStatement.CapabilityStatementRestResourceComponent>() {
							@Override
							public int compare(
									org.hl7.fhir.r4.model.CapabilityStatement.CapabilityStatementRestResourceComponent theO1,
									org.hl7.fhir.r4.model.CapabilityStatement.CapabilityStatementRestResourceComponent theO2) {
								org.hl7.fhir.r4.model.DecimalType count1 = new org.hl7.fhir.r4.model.DecimalType();
								List<org.hl7.fhir.r4.model.Extension> count1exts = theO1
										.getExtensionsByUrl(RESOURCE_COUNT_EXT_URL);
								if (count1exts != null && count1exts.size() > 0) {
								...
								List<org.hl7.fhir.r4.model.Extension> count2exts = theO2
										.getExtensionsByUrl(RESOURCE_COUNT_EXT_URL);
								if (count2exts != null && count2exts.size() > 0) {
	...
	protected void processAndAddLastClientInvocation(GenericClient theClient, ResultType theResultType,
			ModelMap theModelMap, long theLatency, String outcomeDescription, CaptureInterceptor theInterceptor,
		...
		try {
			...
			// String requestUrl = lastRequest != null ?
			// lastRequest.getApacheRequest().getURI().toASCIIString() : null;
			// String action = lastRequest != null ?
			// lastRequest.getApacheRequest().getMethod() : null;
			// String resultStatus = lastResponse != null ?
			// lastResponse.getStatusLine().toString() : null;
			// String resultBody =
			// StringUtils.defaultString(theInterceptor.getLastResponseBody());
			//
			...
			// ContentType.get(lastResponse.getEntity()) : null;
			...
			Header[] requestHeaders = lastRequest != null ? applyHeaderFilters(lastRequest.getAllHeaders())
					: new Header[0];
			Header[] responseHeaders = lastResponse != null ? applyHeaderFilters(lastResponse.getAllHeaders())
					: new Header[0];
	...
	/**
	 * A hook to be overridden by subclasses. The overriding method can modify the
	 * error message based on its content and/or the related exception.
	 *
	 * @param theErrorMsg
	 *            The original error message to be displayed to the user.
	 * @param theException
